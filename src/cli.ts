...
import {
	TypesGenerator, GraphReferenceResolver 
} from './generator';
...
import {
	resolveGraphTypeReference, TypeGraphImpl 
} from './graph';
import {
...
function run (options: CLIOptions): number {
	...
	const referenceResolver: GraphReferenceResolver = (simpleName, anchor) => {
		const refResult = resolveGraphTypeReference(graph, simpleName, anchor);
	...
	const generator = new TypesGenerator(graph, options.esm, options.outputDir, referenceResolver);
	...
	let registryTypes: { content: string; types: string[] } | undefined;
