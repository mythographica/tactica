...
import { TypeNode, PropertyInfo, AnalyzeResult, AnalyzeError, DefinitionInfo, UsageInfo, ConstructorParamInfo, DriftReport } from './types';
...
export class MnemonicaAnalyzer {
	 ...
	 */
	private getTypeChecker(): ts.TypeChecker | undefined {
	 ...
	 */
	private resolvePropertiesFromTypeNode(typeNode: ts.TypeNode): Map<string, PropertyInfo> | undefined {
	 ...
	 */
	private resolveConstructorParamsFromTypeNode(typeNode: ts.TypeNode): ConstructorParamInfo[] | undefined {
		const checker = this.getTypeChecker();
		if (!checker) return undefined;
		...
		if (checker.isTupleType(type)) {
			const tupleArgs = (type as ts.TypeReference).typeArguments ?? [];
	...
	detectDrift(): DriftReport[] {
		...
		for (const node of this.graph.allTypes.values()) {
			...
			// pattern, not drift.
			if (!inferredHasContent) continue;
			...
			for (const [ key, declaredProp ] of declared) {
				...
				if (
					declaredProp.type !== inferredProp.type &&
	...
	private inferTypeFromInitializer(initializer: ts.Expression, dataTypeMap?: Map<string, string>, classPropertyTypes?: Map<string, string>): string {
		 ...
		 */
		private extractDeclaredFromGenerics(call: ts.CallExpression): {
			properties?: Map<string, PropertyInfo>;
		 ...
		 */
		private extractDeclaredFromThisAnnotation(call: ts.CallExpression): Map<string, PropertyInfo> | undefined {
			const handlerArg = call.arguments[1];
			if (!handlerArg) return undefined;
			if (!ts.isFunctionExpression(handlerArg) && !ts.isArrowFunction(handlerArg)) return undefined;
