...
- **Points (v1 contract, unchanged):** framework lifecycle crossroads detected syntactically (no type checker, no dive dependency), driven by the merged plugin vocabulary: heritage (`implements` a plugin-listed interface), decorator sites (plugin-listed decorators on classes, methods, and method parameters — parameter sites take the enclosing `method:Class.method` scope — incl. `new X(...)` args and plugin-listed factory calls in decorator args, `@UsePipes(mvp.forType(Dto))` / `@Body(mvp.forType(Entity, Dto))` via `decoratorArgFactories: { forType: { kind, targetArg? } }`, target class at `targetArg`, default 0), provider-token object literals (`{ provide: TOKEN, useClass: X }`, scope `global`), and `consumer.apply(Mw).forRoutes(...)` inside `configure()` (scope `module`; requires a plugin with `middlewareWiring: true`). No plugins → `points: []`.
...
- `typePath` is the mnemonica fullPath when knowable without the type checker: `new KnownType(…)`, `new instance.Sub.Type(…)` chained through a tracked variable's typePath, an underscore annotation (`UserEntity_UserResponse` → dotted), or a `lookup('Path')` initializer (literal paths only). Lookup initializers run the same tier law as the analyzer: the scope chain's receiver binding first (value scope, innermost wins), then the resolver's delegate — `src/cli.ts` wires it to `MnemonicaAnalyzer.resolveLookupCallPath()`, so import-anchored and chain-relative lookups (`Holder.lookup('Token')` with an imported `Holder`) land the same fullPath the usages pass resolved; only graph-known results are accepted, and ambiguous names resolve to nothing rather than guessing.
...
- Getters: `getGraph`, `getDefinitions`, `getUsages`, `getEDSUsages`, `getFlowUsages`, `getInstrumentationPoints`, `getResolutionErrors` (fatal duplicate/ambiguous graph references, unresolved literal `lookup()` paths, and ambiguous plain-TS references per the hard-fail law).
...
- `define('TypeName', handler)` — root or nested via `Parent.define(…)` or chained `define(…).define(…)`. A const holding a multi-hop initializer binds the LAST hop (`const Branch = Root.define('A', …).define('B', …)` → `Branch` denotes `Root.A.B` — define() returns the defined type's constructor), so children land where the runtime nests them.
- **Self-referencing root ctor annotation (recommended call-site pattern):** the handler's `this` may be annotated with the generated instance alias, commonly an intersection (`type TWidgetInstance = TWidgetArgs & TGeneratedRootInstance`, imported from the generated types file). The annotation is ergonomic-only — never emitted (no circularity), members never expanded — so the root's fields must be carried by the constructor body (`Object.assign(this, args)` or explicit assignments); a root-level child then sees them through the ProtoFlat chain (F21). Gives zero-cast assignability of `new (lookup('Widget'))(args)` to the instance type.
...
- **No `getTypeChecker()` binding** — referenced types in constructor signatures resolve through the importing file's own imports (relative + tsconfig-paths, barrels chased), then locals, then a unique program-wide declaration; failure (ghost names, unextractable constructs) emits `unknown`, never a bare name (generated types.ts has no imports). Ambiguity — several project-source files declaring the name with no import in the referencing file to disambiguate — is FATAL like the graph law: the reference site plus every declaration site is printed, the CLI exits non-zero, and no output is written. Package-declared same-named types never count (a user-local declaration always wins; `.d.ts`/`node_modules` files are excluded from both resolution and the law), and same-file interface merging is not ambiguity (one declaration kept per name per file, last wins). Referenced-type expansion walks class/interface `extends` chains (depth-capped, cycle-guarded, through the same import-aware machinery; external or mixin bases are not followed — their inherited fields stay absent), and `typeof` fields resolve only against module const arrays with statically visible literal elements (any assertion spelling — `as const` / `satisfies` / `<const>[…]`; signed numerics keep their sign; the index suffix is consumed entirely — `typeof arr[number]` emits exactly the union, never `'last'[number]`). The value-level twin — element access over a const-asserted literal array (`(<const>[…])[0]`, `([…] as const)[1]`, tracked `x[0]`) — infers the element literal under the same rules (non-numeric/out-of-range/spread/non-literal elements and general `<T>` assertions stay `unknown`). Every other typeof source — or any indexed-access target that fails to resolve — degrades the whole expression to `unknown`; an index suffix on a fallback type (`unknown[number]`) is invalid TypeScript and is never emitted. **Return-annotation emission law (0.2.0 restored):** in method/member return annotations, inner type arguments resolving to graph types emit the generated aliases; `InstanceType<typeof X>` (directly or via a local alias) drops the wrapper and emits the generated alias — X unresolvable degrades the WHOLE expression to `unknown`, never `InstanceType<unknown>` (invalid TS, TS2344); a non-graph outer generic that is ambient/lib (not declared in any analyzed project file — `MapIterator`, lib helpers) emits verbatim with inner aliases resolved; a project-local non-graph name stays `unknown` (self-contained types.ts carries no imports; same-named ambiguity is the fatal class). Constructor-signature parameters keep their accepted mirror policy and are NOT covered by this law. Name-based heuristics, not the real symbol table. Qualified namespace references (`models.Type`, `holders.Inner.Crate`, `barrel.Deep.Gadget`) resolve one segment at a time — namespace declarations, `export * as ns from '…'` barrels, and named namespace re-exports are followed; ambient string-named modules and field-less type constructs are not. Mnemonica-graph references are the exception: they resolve path-aware (value scope, imports, relative-first chain, collection roots, program-wide-unique) and ambiguity or same-namespace duplicates are fatal — the CLI prints every location, exits non-zero, and writes no output. Literal `lookup()` paths join the same law: a path matching no graph type is what the runtime answers with `undefined`, so it is fatal as well (with same-named off-root types listed as candidates); non-literal arguments stay best-effort. Resolution is authoritative from the usages pass: `resetUsages()` clears `processedCalls` so pass 2 re-extracts every constructor against the complete graph (pass 1 sees forward references as soft `none`).
